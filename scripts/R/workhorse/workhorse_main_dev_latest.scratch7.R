...
if (opt$build_manifest) {
  ...
  if (opt$fresh || !valid_time_stamp(stamp_vec)) {
    ...
    aqp_cgn_tib2 <- bsp_mapping_workflow(
    ...
    bsp_tib <-
      run_bsmap(ref=run$gen_ref_fas, can=run$aqp_prb_fas,
                out=run$bspDir, exe=opt$bsmap_exe,
                sort=TRUE, light=TRUE, reload=opt$reload,
                verbose=opt$verbose,tt=pTracker)
    
    aqp_bsp_tib <-
      join_bsmap(bsp=bsp_tib, ord=aqp_ord_tib, cgn=run$cgn_bed_tsv,
  ...
  if (opt$fresh || !valid_time_stamp(stamp_vec)) {
    ...
    aqp_cgn_tib <-
      assign_cgn(ord=aqp_ord_tib, bsp=aqp_bsp_tib, 
                 seq=aqp_seq_tib, can=run$canonical_csv, 
