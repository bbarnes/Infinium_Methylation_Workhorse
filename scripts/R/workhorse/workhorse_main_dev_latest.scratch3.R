...
if (!is.null(run$cgn_bed_tsv)) {
  ...
  Cgd_tib <- suppressMessages(suppressWarnings(
    readr::read_tsv(run$cgn_bed_tsv, 
                    col_names=names(cgd_bed_cols$cols),
                    col_types=cgd_bed_cols) )) 
  
  %>% 
    dplyr::mutate(Cgd_tib=paste0("chr",Cgd_tib))
  
  # cgn_bed_tib <- cgn_bed0_tib %>%
  #   dplyr::mutate(Imp_Pos=Imp_Pos+1) %>%
  #   purrr::set_names()
}
...
if (opt$build_manifest) {
  ...
  if (opt$fresh || !valid_time_stamp(stamp_vec)) {
    ...
    bsp_cgn_tib <-
      run_bsmap(ref=run$gen_ref_fas, can=run$aqp_prb_fas,
                ...
                sort=TRUE, light=TRUE, reload=TRUE,
                verbose=opt$verbose,tt=pTracker)
    
    # TBD:: The writing function should be moved to run_bsmap()
    bsp_cgn_cnt <- 
      safe_write(bsp_cgn_tib,"csv",run$aqp_bsp_tsv, funcTag=par$prgmTag,
                 verbose=opt$verbose,tt=pTracker)
    
    # Keep this::
    #
    aqp_bsp_tib <-
      join_bsmap(bsp=bsp_cgn_tib, add=aqp_add_tib, cgn=cgn_bed_tib,
    ...
    if (FALSE) {
      ...
      # Build off by one pairs::
      cgn_bed0_tib <- cgn_bed_tib
      cgn_bed1_tib <- cgn_bed_tib %>% dplyr::mutate(Imp_Pos=Imp_Pos+1) 
      
      # Join with probe extension alignments
      bsp_cgn0_tib <- bsp_cgn_tib %>% dplyr::inner_join(cgn_bed0_tib, by=c("Bsp_Chr"="Imp_Chr", "Bsp_Pos"="Imp_Pos"))
      bsp_cgn1_tib <- bsp_cgn_tib %>% dplyr::inner_join(cgn_bed1_tib, by=c("Bsp_Chr"="Imp_Chr", "Bsp_Pos"="Imp_Pos"))
      
      join_tib <- dplyr::inner_join( bsp_cgn0_tib, bsp_cgn1_tib, by="Aln_Key_Unq", suffix=c("0","1") )
      join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1)
      bsp_cgn_tib %>% dplyr::select(Address,Bsp_Chr,Bsp_Pos)
      
      dplyr::bind_rows(
        join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter(!is.na(Imp_Cgn0) &  is.na(Imp_Cgn1)),
        join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter( is.na(Imp_Cgn0) & !is.na(Imp_Cgn1)),
        join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter( is.na(Imp_Cgn0) &  is.na(Imp_Cgn1)),
        join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter(!is.na(Imp_Cgn0) &!is.na(Imp_Cgn1))
      )
      #
      # Intersting the alternation::    
      # join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter(!is.na(Imp_Cgn0) | !is.na(Imp_Cgn1))
      # join_tib %>% dplyr::select(Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter(!is.na(Imp_Cgn0) |  is.na(Imp_Cgn1))
      #
      ...
      #   Variables: Ord_Din,
      #
      bsp_mis_tib <- join_tib %>% dplyr::select(Aln_Key_Unq,Address0,Address1,Bsp_Pos0,Bsp_Pos1,Imp_Cgn0,Imp_Cgn1) %>% dplyr::filter( is.na(Imp_Cgn0) &  is.na(Imp_Cgn1))
      bsp_MIS_tib <- bsp_cgn_tib %>% dplyr::filter(Aln_Key_Unq %in% bsp_mis_tib$Aln_Key_Unq)
      
      v <- c("Ord_Din","Bsp_Rank")
      v <- c("Ord_Din","Bsp_Din_Bsc")
      bsp_MIS_tib  %>% dplyr::select(all_of(v)) %>% dplyr::group_by_all() %>% dplyr::summarise(Count=n(), .groups="drop") %>% print(n=1000)
      bsp_HIT0_tib %>% dplyr::select(all_of(v)) %>% dplyr::group_by_all() %>% dplyr::summarise(Count=n(), .groups="drop") %>% print(n=1000)
      bsp_HIT1_tib %>% dplyr::select(all_of(v)) %>% dplyr::group_by_all() %>% dplyr::summarise(Count=n(), .groups="drop") %>% print(n=1000)
      
      # These make sense we'll copy their distributions with the bad...
      bsp_HIT0_tib <- join_tib %>% dplyr::filter(!is.na(Imp_Cgn0) &  is.na(Imp_Cgn1)) %>% purrr::set_names(nam0_vec)
      bsp_HIT1_tib <- join_tib %>% dplyr::filter( is.na(Imp_Cgn0) & !is.na(Imp_Cgn1)) %>% purrr::set_names(nam1_vec)
      
      nam0_vec <- bsp_HIT0_tib %>% names() %>% stringr::str_remove("0$")
      nam1_vec <- bsp_HIT0_tib %>% names() %>% stringr::str_remove("1$")
    }
  ...
  if (opt$fresh || !valid_time_stamp(stamp_vec)) {
    ...
    imp_fmt_tib <- imp_des_tib %>% purrr::set_names(
