...
initSesameRaw = function(prefix, platform, manifest, load=FALSE, save=FALSE, rds=NULL, 
  ...
  if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr} Starting; platform={platform}, prefix={prefix}.{RET}"))
  stime <- system.time({
    if (load && !is.null(rds) && file.exists(rds)) {
      ...
      if (verbose>=vt+4) print(manifest)
...
sigsSumToSSheet2 = function(tib, metric='avg',
  ...
  if (verbose>=vt+4) ss_tib %>% as.data.frame() %>% print()
