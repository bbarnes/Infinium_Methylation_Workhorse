...
r_improbe = function(tib,
  ...
  #
  # TBD::[DONE]! Add target strands and extract strands from input tib!
  ...
  if ( ( srsplit && !is.null(srd_key) ) || 
    ...
    stime <- system.time({
      ...
      if (!srd_str %in% valid_srd_vec) {
        stop(glue::glue("{RET}{mssg} ERROR: Invalid SR string={srd_str}!{RET2}"))
        return(ret_tib)
      }
      valid_cos_vec <- c("CO")
      if (!cos_str %in% valid_cos_vec) {
        stop(glue::glue("{RET}{mssg} ERROR: Invalid CO string={cos_str}!{RET2}"))
        return(ret_tib)
      }
      ...
      #
      #
      src_man_tib <- tib %>% dplyr::select(!!ids_sym, !!din_sym, !!seq_sym) %>%
...
validate_templates = function(tib,
  ...
  stime <- base::system.time({
    ...
    ret_key <- glue::glue("src_man_tib({funcTag})")
    ret_cnt <- print_tib(ret_tib,funcTag, verbose,vt+4,tc, n=ret_key)
  })
  etime <- stime[3] %>% as.double() %>% round(2)
  if (!is.null(tt)) tt$addTime(stime,funcTag)
...
bsc_templates = function(tib,
  ...
  stime <- base::system.time({
    ...
    int_key <- glue::glue("tib-Init({funcTag})")
    int_cnt <- print_tib(tib, funcTag, verbose,vt+8,tc, n=int_key)
    ...
    if (!is.null(des_seq_tib)) {
      ...
      ret_cnt <- print_tib(ret_dat[[srdC]],funcTag, verbose,vt+8,tc, n=ret_key)
      ...
      if (srdO %in% srd_vec) {
        ...
        ret_cnt <- print_tib(ret_dat[[srdO]],funcTag, verbose,vt+8,tc,n=ret_key)
      }
