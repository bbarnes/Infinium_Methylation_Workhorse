...
par$prgmTag <- 'manifest_subset_noob'
...
per1_vec     <- splitStrToVec(opt$per1)
per2_vec     <- splitStrToVec(opt$per2)
type_vec     <- splitStrToVec(opt$type)

# if (!is.null(opt$rand)) opt$outDir <- file.path(opt$outDir, paste0("rand",opt$rand))
