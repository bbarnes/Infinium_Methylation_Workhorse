...
get_manifest_list = function(file=NULL, dir=NULL, platform=NULL, version=NULL,
  ...
  stime <- system.time({
    ...
    if (!is.null(file) && file.exists(file)) {
      if (verbose>=vt) 
        cat(glue::glue("[{funcTag}]:{tabsStr} Adding direct manifest[1]={file}.{RET}"))
      path_vec <- c(path_vec,file)
    }
    
    if (!is.null(dir) && dir.exists(dir)) {
      
      if (!is.null(platform) && !is.null(version)) {
        if (verbose>=vt) 
          cat(glue::glue("[{funcTag}]:{tabsStr} Specific search[2]; ",
                         "platform={platform}, version={version}, ",
                         "suffix={suffix}, dir={dir}.{RET}"))
        
        fileName <- paste0(platform,'-',version,suffix)
        filePath <- file.path(dir, fileName)
        if (!file.exists(filePath)) {
          stop(glue::glue("[{funcTag}]: ERROR: Failed to find ",
                          "manifest={filePath}; ",
                          "platform={platform}, version={version}, ",
                          "suffix={suffix}, dir={dir}.{RET}{RET}"))
          return(ret_tib)
        }
        path_vec <- c(path_vec,filePath)
      } else {
        if (verbose>=vt) 
          cat(glue::glue("[{funcTag}]:{tabsStr} Full search[3]; ",
                         "suffix={suffix}, dir={dir}.{RET}"))
        
        curPaths <- list.files(file.path(dir), pattern=suffix, full.names=TRUE)
        path_vec <- c(path_vec,curPaths)
      }
    }
    
    path_cnt <- length(path_vec)
    if (path_cnt==0) {
      stop(glue::glue("{RET}[{funcTag}]: ERROR: Failed to find any files!{RET}{RET}"))
      return(ret_tib)
    }
    ret_tib <- tibble::tibble(path=base::unique(path_vec))
...
load_manifest_list = function(tib, field="path",
  ...
  stime <- system.time({
    ...
    for (ii in c(1:tib_row_cnt)) {
      ...
      if (verbose>=vt+4)
        cat(glue::glue("[{funcTag}]:{tabsStr} Loading manifest[{ii}]={cur_file}...{RET}"))
...
getManifestList_old = function(path=NULL, platform=NULL, manifest=NULL, dir=NULL, 
                               verbose=0,vt=4,tc=1,tt=NULL) {
  funcTag <- 'getManifestList_old'
  tabsStr <- paste0(rep(TAB, tc), collapse='')
  if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr} Starting...{RET}"))
  ...
  stime <- system.time({
    ...
    if (!is.null(path) && file.exists(path)) {
      if (verbose>=vt) 
        cat(glue::glue("[{funcTag}]:{tabsStr} Matched[1] path={path}.{RET}"))
      ...
      if (verbose>=vt) 
        cat(glue::glue("[{funcTag}]:{tabsStr} Searching[2] for platform={platform}, ",
      ...
      if (verbose>=vt) 
        cat(glue::glue("[{funcTag}]:{tabsStr} Sesame Assumed[3] path={path}; dir={dir}.{RET}"))
    ...
    if (is.null(paths) || length(paths)==0) {
      stop(glue::glue("[{funcTag}]: ERROR: Paths length is zero or non-existent!{RET}{RET}"))
      return(ret_dat)
    }
    
    paths_len <- length(paths)
    if (verbose>=vt+4) {
      cat(glue::glue("[{funcTag}]:{tabsStr} paths({paths_len})={RET}"))
      print(paths)
    }
    ...
    for (ii in c(1:paths_len)) {
      ...
      if (verbose>=vt+4) {
        cat(glue::glue("[{funcTag}]:{tabsStr}{TAB} Done; Loading manifest={RET}"))
        print(ret_dat[[cur_key]])
      }
    }
    
    ret_cnt <- ret_dat %>% names %>% length()
  })
  if (verbose>vt+4) print(ret_dat)
  etime <- stime[3] %>% as.double() %>% round(2)
  if (!is.null(tt)) tt$addTime(stime,funcTag)
  if (verbose>=vt) 
    cat(glue::glue("[{funcTag}]:{tabsStr} Done; Return Count={ret_cnt}; elapsed={etime}.{RET}{RET}",
...
bindProbeDesignList = function(list, platform=NULL, version=NULL, 
  ...
  stime <- system.time({
    ...
    sum_tib <- ret_tib %>% 
      ...
      dplyr::summarise(Count=n(), .groups='drop')
    if (verbose>=vt) print(sum_tib, n=base::nrow(sum_tib))
    
    if (!is.null(sumDir)) {
      if (!dir.exists(sumDir)) dir.create(sumDir, recursive=TRUE)
      sum_csv <- file.path(sumDir, paste(funcTag,'core-manifest.summary.csv.gz', sep=del))
...
man_join_prbs = function(man, prbs,
  ...
  stime <- system.time({
    ...
    if (verbose>=vt) 
      cat(glue::glue("[{funcTag}]:{tabsStr} Match Infinium 1 probes.{RET}"))
    ...
    cat(glue::glue("{RET}{RET}-----------{RET}{RET}",
                   "[{funcTag}]:{tabsStr} PRE_SEQ_ID.{RET}-------------{RET}",
                   "{RET}"))
    ret_tib <- 
...
seq48U_to_cgn = function(tib, imp_tsv,name=NULL, outDir=NULL,
                         mat_key='Seq_48U',fresh=TRUE, colA=4,colB=1,del='.',
                         verbose=0,vt=3,tc=1,tt=NULL) {
...
decodeAqpPqcWrapper = function(ord_vec, mat_vec, aqp_vec=NULL, pqc_vec=NULL,
  ...
  stime <- system.time({
    ...
    if (!is.null(name) && !is.null(outDir)) {
      ...
      if (!dir.exists(outDir)) dir.create(outDir, recursive=TRUE)
      mat_tsv <- file.path(outDir, paste(out_name,'manifest.unq.mat.tsv.gz', sep=del) )
      rep_csv <- file.path(outDir, paste(out_name,'manifest.rep.sum.csv.gz', sep=del) )
      sum_csv <- file.path(outDir, paste(out_name,'manifest.prb.sum.csv.gz', sep=del) )
      ret_csv <- file.path(outDir, paste(out_name,'manifest.raw.dat.csv.gz', sep=del) )
      ...
      csv_vec <- c(mat_tsv,rep_csv,sum_csv,ret_csv)
      stp_tib <- check_timeStamps(name=out_name,outDir=outDir,
                                  origin=origin,files=csv_vec,
                                  verbose=verbose,vt=vt+1,tc=tc+1,tt=tt)
    }
    ...
    if (!fresh && !is.null(stp_tib) && stp_tib$isValid[1]) {
      ...
      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      #         Consolidate all Match Files into Single Order Match File::
      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      ...
      #
      if (verbose>=vt)
        cat(glue::glue("[{funcTag}]: Joining Match TSVs(cnt={mat_len})...{RET}") )
      
      mat_tib <- lapply(mat_vec, loadMAT, format=matFormat,skip=matSkip,
                        verbose=verbose,vt=vt+1,tc=tc+1,tt=tt) %>%
        dplyr::bind_rows(.id="AQP") %>% 
        dplyr::mutate(AQP=as.integer(AQP)) %>% 
        dplyr::arrange(-AQP) %>% 
        dplyr::distinct(Address,.keep_all=TRUE)
      readr::write_tsv(mat_tib,mat_tsv)
      ...
      matFormat <- 'new'
      if (verbose>=vt) 
        cat(glue::glue("[{funcTag}]: Wrote Ordered Full Match TSV={mat_tsv}.{RET}{RET}") )
      
      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      #               Load Previous Builds or Create Fresh Build::
      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      
      if (!fresh && !is.null(ret_csv) && file.exists(ret_csv)) {
        if (verbose>=vt) 
          cat(glue::glue("[{funcTag}]: Loading Pre-existing Raw Manifest={ret_csv}...{RET}") )
        ret_tib <- suppressMessages(suppressWarnings( readr::read_csv(ret_csv) ))
        ret_cnt <- ret_tib %>% base::nrow()
      } else {
        ...
        if (!is.null(pqc_man_tib) && !is.null(aqp_man_tib)) {
          ...
          qc_man_tib <- AQP_PQC_QC(aqp=aqp_man_tib, pqc=pqc_man_tib, 
                                   ...
                                   aqp_fix=aqp_man_tib, pqc_fix=pqc_man_tib,
                                   name=paste(name,funcTag, sep='.'),outDir=outDir,
                                   verbose=verbose,vt=vt+1,tc=tc+1,tt=tt)
        }
        
        # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
        #                       Select Return Data (PQC/AQP)::
        # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
    ...
    ret_tib <- tib %>% 
      ...
      dplyr::mutate(
        ...
        col=dplyr::case_when(
          ...
          TRUE ~ NA_character_),
        Probe_Type=stringr::str_sub(Assay_Design_Id,1,2) %>%
          stringr::str_replace('^ne','NE'),
        Probe_Source=NA_character_,
        Next_Base=dplyr::case_when(
          ...
          Infinium_Design==1 & Normalization_Bin=='B' ~ 'G',
          TRUE ~ NA_character_),
        AlleleA_Probe_Length=stringr::str_length(AlleleA_Probe_Sequence),
...
fixOrderProbeIDs = function(tib, field="Probe_Type",
  ...
  stime <- system.time({
    ...
    tib_types <- tib %>% split(.[field_sym])
    tib_names <- names(tib_types)
    if (verbose>=vt+4) print(tib_names)
    ...
    ord_col <- c("Seq_ID","FR","TB","CO","Infinium_Design","Probe_ID","M","U","AlleleA_Probe_Sequence_MIX",
                 ...
                 "Decode_Status_B","BP","AQP",
                 # "Rep_Max","Rep_Num",
                 "AlleleA_Probe_Length","AlleleB_Probe_Length",
                 # "Old_Probe_ID",
                 "Probe_Type","Di","DS","HS","FN")
    ...
    for (type in tib_names) {
      ...
      if (verbose>=vt+4) cat(glue::glue("[{funcTag}]:{tabsStr}{TAB} cur_len={cur_len}...{RET}"))
      ...
      if (type=='cg' || type=='ch' || type=='rp') {
        ...
        cur_tib <- cur_tib %>% mutate(Probe_Type := !!type)
      } else if (type=='mu') {
        ...
        cur_tib <- cur_tib %>% mutate(Probe_Type='mu')
      } else if (type=='rs') {
        ...
        cur_tib <- cur_tib %>% mutate(Probe_Type='rs')
        ...
        cur_tib <- cur_tib %>% dplyr::mutate(
          ...
          tidyr::separate(Probe_ID, into=c('Seq_ID','FR','TB','CO','PD'), sep='_', remove=FALSE) 
        cur_tib <- cur_tib %>% mutate(Probe_Type='BS')
      } else if (type=='NO' || type=='no') {
        cur_tib <- cur_tib %>% dplyr::mutate(
          Di=stringr::str_replace(Probe_ID, '^.*-([ACTG][ACTG])-.*$', '\\$1') %>% stringr::str_remove_all('\\\\')
        ) %>% 
          dplyr::mutate(Probe_ID=stringr::str_replace(Probe_ID, '^NON_', 'NP-') %>% 
                          stringr::str_replace('\\.', '-') %>% stringr::str_replace('_','-') %>% stringr::str_replace('_','-')) %>% 
          tidyr::separate(Probe_ID, into=c('Seq_ID','FR','TB','CO','PD'), sep='_', remove=FALSE) 
        cur_tib <- cur_tib %>% mutate(Probe_Type='NP')
      } else if (type=='NE' || type=='ne') {
        cur_tib <- cur_tib %>% 
          dplyr::mutate(Probe_ID=paste(stringr::str_replace(Probe_ID, '^neg_', 'NG-'),'_I') ) %>% 
          tidyr::separate(Probe_ID, into=c('Seq_ID','FR','TB','CO','PD'), sep='_', remove=FALSE) %>%
          dplyr::mutate(DS=NA)
        cur_tib <- cur_tib %>% mutate(Probe_Type='NG')
      } else {
...
manifestCheckSummary = function(tib,
  ...
  stime <- system.time({
    ...
    test_cnt <- tib %>% base::nrow()
    
    if (verbose>=vt)
      cat(glue::glue("[{funcTag}]: test_cnt={test_cnt}.{RET}"))
    ...
    repU_fail_cnt <- tib %>% 
      dplyr::filter(U %in% ret_tib$Address) %>% base::nrow()
    repM_fail_cnt <- tib %>% 
      dplyr::filter(M %in% ret_tib$Address) %>% base::nrow()
...
loadORD = function(file, format='old', skip=8, guess=50000, trim=FALSE,
  ...
  stime <- system.time({
    ...
    ret_tib <- inferOrderFields(ret_tib, sidx=sidx, plen=plen,
                                verbose=verbose,vt=vt+1,tc=tc+1,tt=tt)
...
decodeToManifest = function(ord, mat, pqc, 
                            platform=NULL, version=NULL, AQP=NULL,
                            ordFormat='old', ordSkip=8,  ordGuess=50000,
                            matFormat='new', matSkip=40, matGuess=1000,
  ...
  stime <- system.time({
    ...
    ord_tib <- ord_tib %>% # dplyr::rename(ord_tib, Probe_ID=Assay_Design_Id) %>% 
...
decodeToManifestWrapper = function(ords, mats, pqcs=NULL, aqps=NULL, 
    ...
    for (idx in c(1:ord_cnt)) {
      ...
      ret_tib <- ret_tib %>% dplyr::bind_rows(
        decodeToManifest(ord=ord_file, mat=mat_file, pqc=pqc_file,
                         platform=platform, version=version, AQP=idx,
