...
plotDmlWithDeltaBeta = function(betas, dml, outDir, label, retData=FALSE, writeCSV=FALSE, minDB,
                                verbose=0,vt=3,tc=1,tt=NULL) {
  funcTag <- 'plotDmlWithDeltaBeta'
  tabsStr <- paste0(rep(TAB, tc), collapse='')
  if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr} Starting...{RET}"))
  
  if (!dir.exists(outDir)) dir.create(outDir, recursive=TRUE)
  
  betas_loci_tib <- crossSampleLociRSquared(betas, verbose=verbose)
  if (retData) return(betas_loci_tib)

  # TBD:: merge loci_byClass_tib
  dml_beta_tib <- dplyr::left_join(dml, betas_loci_tib, by="Probe_ID")
  dml_beta_csv <- file.path(outDir, paste(label, 'dml_topLoci_all_1_join_dBeta.csv.gz', sep='_'))
  dml_beta_png <- file.path(outDir, paste(label, 'dml_topLoci_all',paste0('dB-min-',minDB),plotMax,'_rankByDeltaBeta.png', sep='_'))
  if (writeCSV) readr::write_csv(dml_beta_tib, dml_beta_csv)
  
  dml_loci_join_filter <- dml_beta_tib %>% 
    dplyr::filter(!is.na(NEG_dB2_cnt)) %>% dplyr::filter(!is.na(POS_dB2_cnt)) %>%
    dplyr::filter(!is.na(NEG_beta_sd)) %>% dplyr::filter(!is.na(POS_beta_sd)) %>%
    dplyr::select(Probe_ID,Rank, NEG_nan_per, POS_nan_per, NEG_beta_mu, POS_beta_mu, NEG_dB2_cnt, POS_dB2_cnt,
                  NEG_POS_CSS_q2, NEG_POS_CSS_mu, NEG_POS_CSS_sd, NEG_POS_CSS_per, NEG_POS_CSS_cnt)
  
  #
  # Below gives us a way to plot delta Beta for the top hits ranked my Differentially Methylated Loci::
  #
  dB_dat_tib <- dml_loci_join_filter %>% dplyr::mutate(dB=abs(NEG_beta_mu-POS_beta_mu) ) %>% dplyr::filter(dB>=minDB)
  dB_sub_tib <- dB_dat_tib %>% dplyr::select(Probe_ID, Rank, NEG_beta_mu, POS_beta_mu, dB) %>% head(n=plotMax)
  
  gg <- ggplot2::ggplot(data=dB_sub_tib, aes(x=Rank, y=dB)) +
    ggplot2::geom_point(size=0.1) +
    ggplot2::geom_density2d(alpha=0.8)
  
  # ggplot2::ggsave(dml_beta_png, gg, dpi=opt$dpi)
  
  cat(glue::glue("[Done]: png={dml_beta_png}.{RET}{RET}"))
...
manhattanPlot = function(tib, man, key='Probe_ID', col, outDir, outStr, cutoff, sn=0.000001,
                         verbose=0,vt=3,tc=1,tt=NULL) {
  funcTag <- 'manhattanPlot'
  tabsStr <- paste0(rep(TAB, tc), collapse='')
  if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr} Starting...{RET}"))
  ...
  stime <- system.time({
    key <- rlang::sym(key)
    col <- rlang::sym(col)
    tib <- tib %>% dplyr::select(!!key, !!col)
    
    if (verbose>=vt+4) print(tib)
    pre_nrows <- tib %>% base::nrow()
    perc_tib  <- tib %>% purrr::set_names(c('Probe_ID', 'Pval')) %>% 
      dplyr::group_by(Probe_ID) %>% dplyr::summarise(Fail_Perc=cntPer_gt0(Pval, max=cutoff) )
    if (verbose>=vt+4) print(perc_tib)
    
    plot_tib <- man %>% dplyr::select(Probe_ID, chr_idx, start) %>% 
      dplyr::rename(Chromosome=chr_idx, ProbePos=start) %>% 
      dplyr::inner_join(perc_tib, by="Probe_ID") %>%
      dplyr::mutate( Fail=case_when( Fail_Perc>cutoff ~ 1, TRUE ~ 0 ) ) %>%
      dplyr::arrange(Chromosome, ProbePos) %>% 
      dplyr::group_by(Chromosome) %>% 
      dplyr::mutate(Chrom_Order=row_number()) %>%
      dplyr::rename(SNP=Probe_ID, CHR=Chromosome, BP=Chrom_Order, P=Fail_Perc) %>% 
      dplyr::select(SNP, CHR, BP, P) %>%
      dplyr::ungroup() %>% dplyr::mutate( P=case_when(P==1 ~ P-sn, P==0 ~ P+sn, TRUE ~ P) )

    pos_nrows <- plot_tib %>% base::nrow()
    pass_perc <- round(100*pos_nrows/pre_nrows, 3)
    if (verbose>=vt+4) print(plot_tib)

    plot_count <- plot_tib %>% base::nrow()
    main_str   <- glue::glue("{col} Manhattan Plot, cutoff={cutoff}{RET}pre={pre_nrows}, post={pos_nrows}, pass={pass_perc}%.{RET}")
    
    gg_pdf   <- glue::glue('{outDir}/{outStr}-{col}-subset{plot_count}.pdf')
    pdf(gg_pdf)
    manhattan(plot_tib, main = main_str, chr="CHR", bp="BP", snp="SNP", p="P")
    dev.off()
  })
...
screenTitration = function(tib, summary=FALSE,
  ...
  if (verbose>=vt+4) print(flag_tib)
...
plotBetaMatrix_bySample = function(tib, sample, manifest, minPval, pvalKey, betaKey,
                                   outDir, field='Beta', join='Probe_ID', sort=NULL, joinFull=FALSE,
...
diag_density <- function(data, mapping, fdat=NULL, group, minPval, only, field='Beta',
                         alpha=0.8,alpha_lab=0.3,size=0.5,wsize=3, non.ref=TRUE,
  ...
  sum_str <- getSummaryLabel(fdat, group=group, field='Pval', minVal=minPval, rm.nan=TRUE)
...
geomDensity1d_Delta <- function(data, mapping, fdat=NULL, bufs=NULL, group, minDelta, only=NULL,
  ...
  sum_str <- getSummaryLabel(fdat, group=group, field='Delta', minVal=minDelta, rm.tot=FALSE)
...
getSummaryLabel = function(tib, pad=' ', group, field, minVal=NULL, rm.tot=FALSE, rm.nan=FALSE,
                           verbose=0,vt=4,tc=1) {
  ...
  for (ii in c(1:nrows)) {
    ...
    # cat("\tncols=",ncols,", LAB_STR=",lab_str,', cur_str=',cur_str,'.\n\n', sep='')
  }
...
buildDmlFromBeta_OLD = function(betas, cols, csv=NULL, rds=NULL, isBoolean=FALSE,
                                buildDMR=FALSE, writeCSV=FALSE, writeRDS=FALSE,
                                verbose=0,vt=4,tc=1,tt=NULL) {
  funcTag <- 'buildDmlFromBeta'
  tabsStr <- paste0(rep(TAB, tc), collapse='')
  if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr} Starting...{RET}"))
  
  stime <- system.time({
    if (verbose>=vt+4) print(betas)
    
    dml_dat <- NULL
    dml_dat_names <- betas %>% purrr::set_names(cols)
    if (isBoolean) dml_dat_names <- dml_dat_names %>% dplyr::select(Probe_ID, starts_with('NEG_'), starts_with('POS_'))
    
    dml_dat$betas <- dml_dat_names %>% tibble::column_to_rownames("Probe_ID") %>% as.matrix()
    if (verbose>=vt+4) print(dml_dat_names)
    ...
    if (verbose>=vt+4) dml_dat$betas %>% head(n=2) %>% print()
    if (verbose>=vt+4) dml_dat$sampleInfo %>% head(n=2) %>% print()
    if (verbose>=vt+4) print(dml_dat$sampleInfo)
    
    # DML::
    # if (!is.null(csv) && file.exists(csv)) {
    #   dml_tib <- readr::read_csv(csv)
    if (!is.null(rds) && file.exists(rds)) {
      dml_lst <- readr::read_rds(rds)
    } else {
      dml_lst <- sesame::DML(dml_dat$betas, dml_dat$sampleInfo, ~type)
      
      dml_tib <- sesame::topLoci_Local(dml_lst[[1]]) %>% as.data.frame() %>% rownames_to_column('Probe_ID') %>%
      ...
      if (!is.null(csv) && writeCSV) {
        if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr}{TAB} Writing DML-1(CSV)={csv}.{RET}"))
        readr::write_csv(dml_tib, csv)
      }
      if (!is.null(rds) && writeRDS) {
        if (verbose>=vt) cat(glue::glue("[{funcTag}]:{tabsStr}{TAB} Writing DML-1(RDS)={rds}.{RET}"))
        readr::write_rds(dml_lst, rds, compress='gz')
      }
    }
    
    # DMR:: NOT READY OR NEEDED YET!!!
    if (FALSE && buildDMR) {
      dmr <- sesame::DMR(dml_dat$betas, dml_dat$sampleInfo, ~type, cf=dml)
      dmr_topSegs1_tib <- sesame::topSegments(dmr[[1]]) %>% tibble::as_tibble() %>% purrr::set_names(stringr::str_replace_all(names(.), '\\.', '_'))  %>%
        purrr::set_names(names(.) %>% stringr::str_replace_all("[^[A-Za-z,]]+", "_") %>% stringr::str_remove('_$') ) %>%
        dplyr::mutate_if(is.double, list(round), 6) %>%
        dplyr::mutate(Rank=row_number())
      
      dmr_topSegs1_csv <- file.path(curOutDir, paste(subType, 'dmr_topSegs_all_1.csv.gz', sep='_'))
      dmr_topSegs1_rds <- file.path(curOutDir, paste(subType, 'dmr_topSegs_all.rds', sep='_'))
      readr::write_csv(dmr_topSegs1_tib, dmr_topSegs1_csv)
      if (writeRDS) readr::write_rds(dmr, dmr_topSegs1_rds, compress='gz')
    }
  })
  etime <- stime[3] %>% as.double() %>% round(2)
  if (!is.null(tt)) tt$addTime(stime,funcTag)
