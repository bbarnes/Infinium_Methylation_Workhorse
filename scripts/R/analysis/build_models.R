...
if (args.dat[1]=='RStudio') {
  ...
  if (FALSE) {
    ...
    opt$datDir <- paste(
      file.path(par$topDir, "scratch/RStudio/merge_builds", par$platform,opt$classVar,opt$workflow),
      # file.path(par$topDir,"scratch",par$runMode,"merge_builds",opt$runName,
      #           # par$platform,par$version,
      #           # opt$classVar,
      #           opt$classVar,
  ...
  option_list = list(
    ...
    make_option(c("--swaps"), type="character", default=opt$swaps,
                help="List of swap percentages for labels used in machine learning steps. Comma seperated list. [default= %default]", metavar="character"),
...
seed_vec <- splitStrToVec(opt$seeds)
swap_vec <- splitStrToVec(opt$swaps)

mergeDirs_vec   <- splitStrToVec(opt$datDir)

lociBetaKey_vec <- splitStrToVec(opt$lociBetaKey)
lociPvalKey_vec <- splitStrToVec(opt$lociPvalKey)
lociPvalMin_vec <- splitStrToVec(opt$lociPvalMin)

featuresCsv_vec <- splitStrToVec(opt$featuresCsv)
featuresDml_vec <- splitStrToVec(opt$featuresDml)
...
# TBD:: Do we really need this variable (i.e. exp_var) anymore???
...
for (betaKey in lociBetaKey_vec) {
  for (pvalKey in lociPvalKey_vec) {
    for (pvalMin in lociPvalMin_vec) {
      ...
      cur_opts_dir <- file.path(opt$outDir, dirName)
      if (!dir.exists(cur_opts_dir)) dir.create(cur_opts_dir, recursive=TRUE)
      
      if (opt$verbose>0)
        cat(glue::glue("[{par$prgmTag}]: Built; cur_opts_dir={cur_opts_dir}!{RET}") )
      
      if (opt$clean) unlink(list.files(cur_opts_dir, full.names=TRUE))
      
      cTracker <- timeTracker$new(verbose=opt$verbose)
      
      # Defined Output files::
      beta_masked_rds <- file.path(cur_opts_dir, paste(outName,'beta_masked_mat.rds', sep='.') )
      pval_masked_rds <- file.path(cur_opts_dir, paste(outName,'pval_masked_mat.rds', sep='.') )
      index_masks_csv <- file.path(cur_opts_dir, paste(outName,'beta_masked_idx.csv.gz', sep='.') )
      class_ss_csv <- file.path(cur_opts_dir, paste(outName,'ClasSampleSheet.sorted.csv.gz', sep='.') )
      ...
      if (FALSE) {
        ...
        tmp_join_tib %>% dplyr::filter(Sample_Class_25 != Sample_Class_50)
        tmp_join_tib %>% dplyr::filter(Sample_Class_25 != Sample_Class_75)
        tmp_join_tib %>% dplyr::filter(Sample_Class_50 != Sample_Class_75)
      }

      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      #                     Loop Over Label Randomization::
      # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
      
      #
      # TBD:: Pass all parameters to summary file!!!
      #
      # opt$single <- TRUE
      for (lab_swap_idx in c(1:length(swap_vec))) {
        lab_swap_per <- swap_vec[lab_swap_idx]
        
        cur_swap_ss_tib <- rand_ss_list[[lab_swap_idx]]
        
        # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
        #        Build Current Output Directory:: by Label Randomization
        # ----- ----- ----- ----- ----- -----|----- ----- ----- ----- ----- ----- #
        
        cur_swap_dir <- file.path(cur_opts_dir, paste0("label-",lab_swap_per))
        cur_plot_dir <- file.path(cur_swap_dir,'plots')
        if (!dir.exists(cur_swap_dir)) dir.create(cur_swap_dir, recursive=TRUE)
        if (!dir.exists(cur_plot_dir)) dir.create(cur_plot_dir, recursive=TRUE)
        
        full_dml_csv <- file.path(cur_swap_dir, paste(outName,'full-dml.csv.gz', sep='.') )
        full_dbl_csv <- file.path(cur_swap_dir, paste(outName,'full-dbl.csv.gz', sep='.') )
        
        dml_opt_csv  <- file.path(cur_swap_dir, paste(outName,'program-options.csv', sep='.') )
        dml_par_csv  <- file.path(cur_swap_dir, paste(outName,'program-parameters.csv', sep='.') )
        dml_time_csv <- file.path(cur_swap_dir, paste(outName,'time-tracker.csv.gz', sep='.') )
        ...
        labs_idx_vec <- cur_swap_ss_tib %>% dplyr::pull(!!class_idx) %>% as.vector()
        ...
        #     - cur_swap_ss_tib %>% dplyr::group_by(Experiment_Key) %>% dplyr::summarise(Exp_Pass_avg=mean(!!sam_pval_name_sym, na.rm=TRUE), Exp_Pass_max=max(!!sam_pval_name_sym, na.rm=TRUE))
        #
        #  4. Load signal sets
        #  5. Plot signal sets
        #
        if (opt$plot_pairs) {
          ...
          man_path_csv <- NULL
          man_platform <- cur_swap_ss_tib %>% dplyr::distinct(platformUsed) %>% head(n=1) %>% dplyr::pull(platformUsed)
          man_version  <- cur_swap_ss_tib %>% dplyr::distinct(platVersUsed) %>% head(n=1) %>% dplyr::pull(platVersUsed)
          ...
          # cur_swap_ss_tib <- cur_swap_ss_tib %>% dplyr::mutate(Experiment_Key=!!class_two_sym) # %>% dplyr::select(Experiment_Key)
          #
          # if (length(grep(class_two_str, names(cur_swap_ss_tib))) != 0) {
          #   cur_swap_ss_tib <- cur_swap_ss_tib %>% 
          #     dplyr::mutate(!!class_var := paste(!!class_two_sym,!!class_var, sep='_')) # %>% dplyr::select(!!class_var) %>% as.data.frame()
          # }
          
          plotSheet_tib <- cur_swap_ss_tib %>% 
          ...
          for (cIdx in c(1:length(ss_class_keys))) {
            ...
            r2_raw_pdf  <- file.path(cur_plot_dir, paste('r-squared.all-methods.raw',class_key,'pdf', sep='.'))
            r2_norm_pdf <- file.path(cur_plot_dir, paste('r-squared.all-methods.normalized',class_key,'pdf', sep='.'))
            ...
            for (idxA in c(1:length(ss_exp_keys))) {
              for (idxB in c(1:length(ss_exp_keys))) {
                if (idxA<idxB) {
                  ...
                  red_ss_tibA <- reduceSortedTib(tib=cur_ss_tibA, n=3)
                  red_ss_tibB <- reduceSortedTib(tib=cur_ss_tibB, n=3) %>% 
        ...
        if (opt$buildDml) {
          if (!opt$clean && file.exists(full_dml_csv) &&
            ...
            samp_dml_tib <- cur_swap_ss_tib %>% dplyr::rename(Class_Var = !!class_var)
        ...
        if (opt$buildDbl) {
          ...
          if (!opt$clean && file.exists(full_dbl_csv) &&
            ...
            if (TRUE) {
              ...
              #
              sample_cnt_tib <- cur_swap_ss_tib %>% 
              ...
              sample_key_vec <- cur_swap_ss_tib %>% 
                dplyr::pull(!!class_var) %>% unique()
              sample_cnt_vec <- cur_swap_ss_tib %>% 
                dplyr::group_by(!!class_var) %>% 
                dplyr::summarise(Count=n(), .groups="drop") %>% 
                dplyr::pull(Count)
        ...
        if (opt$buildModels) {
          ...
          for (dmlSize in featuresDml_vec) {
            ...
            cur_dml_dir <- file.path(cur_swap_dir, dml_cur_str)
            ...
            for (seed_val in seed_vec) {
              ...
              if (opt$cluster) {
                ...
                cur_time_csv <- file.path(cur_swap_dir, paste(curName,'time-tracker.csv.gz', sep='.') )
